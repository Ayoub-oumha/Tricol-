...
## 10. User Registration via API

You can register users directly via the application API, which creates them in Keycloak.

### Register New User

**POST** `http://localhost:8081/api/v1/auth/keycloak/register`
...
```bash
curl -X POST "http://localhost:8081/api/v1/auth/keycloak/register" \
  -H "Content-Type: application/json" \
  -d '{
    "username": "new_magasinier",
    "email": "mag2@tricol.com",
...
Invoke-RestMethod -Uri "http://localhost:8081/api/v1/auth/keycloak/register" `
    -Method POST `
...
```json
{
    ...
    "keycloakUserId": "929c81af-15ad-4268-ba79-f5889dff3e69",
    "username": "new_magasinier",
    "email": "mag2@tricol.com"
}
```
...
**POST** `http://localhost:8081/api/v1/auth/keycloak/login`
...
curl -X POST "http://localhost:8081/api/v1/auth/keycloak/login" \
  -H "Content-Type: application/json" \
  -d '{"username": "new_magasinier", "password": "magasinier123"}'
```
...
```json
{
    ...
    "refreshToken": "eyJhbGciOiJIUzUxMiIsInR5cC...",
    "expiresIn": 300,
...
### Keycloak Auth Endpoints (via Application)

| Method | Endpoint | Description |
|--------|----------|-------------|
| POST | `/api/v1/auth/keycloak/register` | Register new user in Keycloak |
| POST | `/api/v1/auth/keycloak/login` | Login and get tokens |
| POST | `/api/v1/auth/keycloak/refresh-token` | Refresh access token |

### Direct Keycloak Endpoints
